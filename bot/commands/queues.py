...
async def _seed_unrated_on_add(ctx, member, queue):
	""" Seed unrated players when they join a ranked queue. """
	if queue.cfg.ranked:
		await ctx.qc.rating.seed_if_unrated(
			member,
			rating=ctx.qc.cfg.rating_seed_on_add,
			reason="seed on add"
		)
...
async def add(ctx, queues: str = None):
	...
	for q in t_queues:
		...
		if qr[q] == bot.Qr.QueueStarted:
			await _seed_unrated_on_add(ctx, ctx.author, q)
	...
	if bot.Qr.Success in qr.values():
		for q, response in qr.items():
			if response == bot.Qr.Success:
				await _seed_unrated_on_add(ctx, ctx.author, q)
...
async def add_player(ctx, player: Member, queue: str):
	...
	if resp == bot.Qr.Success:
		await _seed_unrated_on_add(ctx, p, q)
