...
class QueueChannel:
	...
	cfg_factory = CfgFactory(
		...
		variables=[
			...
			Variables.IntVar(
				"rating_seed_on_add",
				display="Seed unrated players on add",
				section="Rating",
				description="Set rating value for unrated players when they are added to a ranked queue.",
				default=500,
				verify=lambda x: 0 <= x <= 10000,
				verify_message="Seed rating must be between 0 and 10000",
				notnull=True
			),
			Variables.IntVar(
				"rating_deviation",
