...
class BaseRating:
	...
	async def seed_if_unrated(self, member, rating, deviation=None, reason="seed on add"):
		""" Seed member rating only when there is no rating yet. Returns True if seeded. """
		old = await db.select_one(
			('rating', 'deviation'), self.table,
			where=dict(channel_id=self.channel_id, user_id=member.id)
		)
		if old and old['rating'] is not None:
			return False

		rating = max(0, int(rating))
		deviation = deviation or self.init_deviation
		...
		if not old:
			...
			await db.update(
				self.table,
				dict(rating=rating, deviation=deviation),
		...
		await db.insert(
			...
			dict(
				...
				deviation_change=deviation-old['deviation'], match_id=None, reason=reason
			)
		)
		return True
